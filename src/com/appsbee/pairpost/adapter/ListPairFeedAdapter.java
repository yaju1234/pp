...
public class ListPairFeedAdapter extends ArrayAdapter<Discoder> {
    ...
    private TabPairFeedFragment tabPairFeedFragment;
    private  Dialog dlg_comments;
    private  Dialog dlg_list;

    public ListPairFeedAdapter(TabPairFeedFragment tabPairFeedFragment,Activity activity, int textViewResourceId) {
	super(activity, textViewResourceId);
	this.mItems = Constant.feedArr;
	this.activity =activity;
	this.tabPairFeedFragment = tabPairFeedFragment;
	...
	if (v == null) {
		...
		v = vi.inflate(R.layout.gallery_item_pair_feed, null);
		...
		mHolder.videoPlayer1 = (CustomVideoView) v.findViewById(R.id.youtube_player1);
		mHolder.videoPlayer2 = (CustomVideoView) v.findViewById(R.id.youtube_player2);
	...
	mHolder.ivUserPic.setOnClickListener(new OnClickListener() {
		...
		tabPairFeedFragment.mCallback.onTabPairFeedListener(mItems.get(position).getId());
	    }
	});
	...
	mHolder.videoPlayer1.setPlayPauseListener(new PlayPauseListener() {
	...
	mHolder.videoPlayer1.setOnCompletionListener(new OnCompletionListener() {
	...
	mHolder.videoPlayer2.setPlayPauseListener(new PlayPauseListener() {
	...
	mHolder.videoPlayer2.setOnCompletionListener(new OnCompletionListener() {
    ...
    public void likeDlg(String totallike) {
   	...
   	PairFeedLikeAdapter listLikeAdapter = new PairFeedLikeAdapter(ListPairFeedAdapter.this,activity,R.layout.row_like,mLikeArr);
   	...
   	dlg_list.setOnDismissListener(new OnDismissListener() {
   		...
   		likeflag = true;
   		ListPairFeedAdapter.this.notifyDataSetChanged();
    ...
    public void commentDlg( final String post_id,final String totalcomment) {
  	...
  	PairFeedCommentAdapter listCommentAdapter = new PairFeedCommentAdapter(pos,lbl_total_comments,ListPairFeedAdapter.this,Constant.userId,post_id,activity,R.layout.row_comments,mCommentArr);
	...
	public CustomVideoView videoPlayer1, videoPlayer2;
...
public class LikeListAsyncTask extends    AsyncTask<String, Void, Boolean> {
	...
	protected void onPostExecute(Boolean result) {
	    super.onPostExecute(result);
	    if(result){
		Constant.feedArr.get(pos).setTotalLike(totallike);
		if(type.equalsIgnoreCase("dislike")){
		    Constant.feedArr.get(pos).setLike(false);
		    
		}else{
		    Constant.feedArr.get(pos).setLike(true);
		}
	...
	String comment_id = "";
  	...
  	protected void onPostExecute(Boolean result) {
  	   ...
  	   if(result){
  	     ...
  	     Constant.feedArr.get(pos).setTotalComments(""+mCommentArr.size());
	...
	tabPairFeedFragment.mCallback.onTabPairFeedListener(id);
    }
	...
	@Override
  	...
  	protected void onPostExecute(Boolean result) {
  	   ...
  	   if(result){
  		   ...
  		   if(mItems.get(i).getPairpostId().equalsIgnoreCase(postid_id)){
